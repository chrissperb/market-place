...
const isInternalPath = (p) => typeof p === 'string' && p.startsWith('/')
...
export default function Login() {
  ...
  const next = isInternalPath(rawNext) ? rawNext : '/'
      ...
      <div className="rounded-3xl bg-slate-800 p-8 ring-1 ring-slate-700">
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-5" noValidate>
          ...
          {(localError || error) && (
            <p
              role="alert"
              className="rounded-lg bg-red-500/10 px-4 py-2 text-sm text-red-300 ring-1 ring-red-500/40"
            >
              {localError || error}
            </p>
          )}
        ...
        <div className="mt-6 rounded-xl border border-slate-700 bg-slate-900 p-4 text-center">
          ...
          <p className="mt-1 text-sm text-slate-200">
            {demo.email} / <span className="font-mono">{demo.password}</span>
          </p>
          <button
            onClick={fillDemo}
